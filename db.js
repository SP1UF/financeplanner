...
function getDB() {
  if (!dbPromise) {
    dbPromise = new Promise((resolve, reject) => {
      const req = indexedDB.open("FinancePlannerDB", 2); // podbijamy wersję
      req.onupgradeneeded = e => {
        const db = e.target.result;

        // transakcje
        if (!db.objectStoreNames.contains("txns")) {
          db.createObjectStore("txns", { keyPath: "id", autoIncrement: true });
        }
        ...
        if (!db.objectStoreNames.contains("payouts")) {
          db.createObjectStore("payouts", { keyPath: "id", autoIncrement: true });
        }
      };
